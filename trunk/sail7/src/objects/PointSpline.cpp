...
void PointSpline::DrawSpline(QPainter & painter, double const &scalex, double const &scaley, QPoint const &Offset)
{
	painter.save();
...
void PointSpline::Duplicate(void *pSpline)
{
	if(!pSpline) return;
	PointSpline *pPointSpline = (PointSpline*)pSpline;
