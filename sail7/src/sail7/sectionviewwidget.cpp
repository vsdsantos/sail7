...
MainFrame *SectionViewWidget::s_pMainFrame;
bool SectionViewWidget::s_bCurrentOnly=false;


SectionViewWidget::SectionViewWidget(SailDlg *pSailDlg) : QWidget(pSailDlg)
{
...
void SectionViewWidget::paintEvent(QPaintEvent *)
{
...
void SectionViewWidget::PaintLegend(QPainter &painter)
{
    ...
    {
        QString strtwist = QString("Section chord = %1").arg(m_pSailSection->Chord()*s_pMainFrame->m_mtoUnit, 6,'f',2) + strlength;
...
void SectionViewWidget::OnZoomIn()
{
    if(!m_bZoomIn)
    {
        ...
        {
            m_bZoomIn = true;
            m_pSailDlg->m_pZoomIn->setChecked(m_bZoomIn);
        }
        else
        {
            ReleaseZoom();
...
void SectionViewWidget::OnZoomYScale()
{
    ...
    m_pSailDlg->m_pZoomYOnly->setChecked(m_bZoomYOnly);
}
...
void SectionViewWidget::OnInsertCtrlPt()
{
    ...
    m_pSailDlg->FillPointTable();
    m_pSailDlg->UpdateView();
}
...
void SectionViewWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    else if(!m_bZoomIn && (event->buttons() & Qt::LeftButton))
    {
        ...
        else if(m_pSailSection && m_pSail->IsNURBSSail())
        {
            ...
            m_pSailDlg->m_pctrlPointTable->selectRow(m_pSailSection->m_iSelect);
...
void SectionViewWidget::contextMenuEvent (QContextMenuEvent * event)
{
    ...
    pCtxMenu->addAction(m_pInsertPoint);
    pCtxMenu->addAction(m_pRemovePoint);
    pCtxMenu->addSeparator();
    pCtxMenu->addAction(m_pSailDlg->m_pZoomYOnly);
    pCtxMenu->addSeparator();
    pCtxMenu->addAction(m_pSailDlg->m_pResetScales);
    pCtxMenu->addAction(m_pSailDlg->m_pResetYScale);
    pCtxMenu->addSeparator();
    pCtxMenu->addAction(m_pCurrentOnly);
...
void SectionViewWidget::resizeEvent(QResizeEvent *)
{
    OnResetScale();
}
